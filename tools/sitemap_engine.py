...
* Indexability is read from an HTML `<meta name="robots">` tag. A page hidden by
  ...
  `exclude`.
* The shared `web_metadata` parser reads single-quoted, double-quoted, and
  unquoted attributes, ignores commented tags, and ignores metadata inside
  inert containers such as `<template>` and `<noscript>`.
...
def load_config(path: Path) -> dict:
    ...
    try:
        web_metadata.parse_origin(base_url)
    except web_metadata.UrlError as error:
        raise EngineRunError(f'config "base_url" is malformed: {error}') from error
...
def parse_meta(html: str) -> tuple[dict[str, str], list[tuple[str, str]]]:
    ...
    for attributes in web_metadata.meta_tags(html):
...
def enumerate_pages(site_root: Path, include: list[str], exclude: list[str]) -> list[Path]:
    ...
    for pattern in include:
        for path in _glob(site_root, pattern):
            if path.is_file():
                if not web_metadata.contained(path, site_root):
                    raise EngineRunError(
...
def _site_root(config: dict, repo_root: Path) -> Path:
    ...
    if not web_metadata.contained(site_root, repo_root):
...
def output_path(config: dict, repo_root: Path) -> Path:
    ...
    if not web_metadata.contained(target.parent, repo_root):
